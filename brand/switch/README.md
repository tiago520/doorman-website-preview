...
The owner selected Switch and all three colorways. This bundle preserves the chosen two-gate geometry. It previews proposed states and motion without changing native product behavior.
...
State masters: assets/switch-STATE-SIZE.svg, where STATE is protected, reconnecting, paused, attention, managed, or signedOut; SIZE is 16, 24, or 32. Static exports work without scripts. switch-state.css applies the proposed motion to classed paths in the HTML.
...
The six-state native reference is Sources/DoormanMenuBar/Glyph.swift, App.swift and Sources/DoormanModel/Model.swift in doorman-mac. The selected brand is approved; new state treatments are presented for implementation. The menu bars are identity applications, not product screenshots.
